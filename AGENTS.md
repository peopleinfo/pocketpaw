...
## Syncing Upstream Code (Beta Fork)

This repository is a **beta fork** of the official PocketPaw upstream. It contains significant custom features, specifically the **Anti-Detect Browser system** integrated into the core dashboard, API, and frontend.

When syncing new code from upstream, AI agents and developers **MUST** preserve our custom code:

1. **Add the upstream remote** (if not already added):

   ```bash
   git remote add upstream https://github.com/pocketpaw/pocketpaw.git
   ```
...
4. **Resolve conflicts**:
   - Upstream updates to `src/pocketpaw/dashboard.py` or frontend templates (`anti_browser.html`, `sidebar.html`, `base.html`) will likely conflict.
   - **Do NOT blindly accept upstream changes** if they overwrite our custom `/api/anti-browser/*` routes or drawer UI hooks.
   - We need **both codes**: integrate upstream bug fixes/features while actively porting our anti-detect browser injections into the merged result.
