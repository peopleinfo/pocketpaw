  ...
  getMethods() {
    return {
      ...
      async searchLibrary() {
        ...
        try {
          const res = await fetch(
            `/api/skills/search?q=${encodeURIComponent(q)}&limit=30`,
          );
          if (res.ok) {
      ...
      async loadFeatured() {
        this.libraryLoading = true;
        try {
          const res = await fetch("/api/skills/search?q=skill&limit=20");
          if (res.ok) {
            const data = await res.json();
      ...
      async installSkill(source) {
        ...
        try {
          ...
          if (res.ok && data.status === "ok") {
            ...
            this.showToast(
              names ? `Installed: ${names}` : "Skill installed!",
              "success",
            );
